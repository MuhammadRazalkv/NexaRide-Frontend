...
const AdminPendingDriver = () => {
    ...
    const [selectedDriver, setSelectedDriver] = useState<IPendingDriver | null>(null);
        ...
        try {
            ...
            if (res.success) {
                ...
                setDrivers((prevDrivers) => prevDrivers
                    ? prevDrivers.filter(driver =>
                        driver.status == 'pending' || driver.vehicleDetails?.status == 'pending'
                    )
                    : []
                );
                    ...
                    prevDrivers
                        ? prevDrivers.map((driver) =>
                            driver.vehicleDetails?._id === selectedVehicleId
                                ? { ...driver, vehicleDetails: { ...driver.vehicleDetails, status: 'rejected' } }
                                : driver
                        )
                        : null
                );
    ...
    return (
        <div className="bg-[#0E1220] min-h-screen flex flex-col items-center  justify-center lg:flex-row p-6 ">
            ...
            <div className=" w-full  max-w-4xl bg-[#1A2036] rounded-lg p-6 shadow-lg border border-[#2C3347]/50">
                <h2 className="text-white text-xl font-semibold mb-4">Pending Drivers</h2>
...

            {
                selectedDriver && (
                    <div className="w-full max-w-md bg-[#1A2036] p-6 rounded-lg shadow-lg border border-[#2C3347]/50 mt-6 md:mt-0 md:ml-6 relative">
                        ...
                        <h3 className="text-white text-xl font-semibold mb-4">Driver Details</h3>
                        <div className="space-y-2">
                            <p className="text-gray-400"><strong>Name:</strong> {selectedDriver.name}</p>
                            ...
                            <p className="text-gray-400">
                                <strong>License Expiry:</strong> {new Date(selectedDriver.license_exp).toLocaleDateString()}
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-3 mt-4">
                            {
                                selectedDriver.status == 'pending' ?
                                    ...
                                    :
                                    <p className="text-white font-bold px-4 text-center ">Driver info verified</p>
                            }
                            <button
                                onClick={() => {
                ...
                showRejectModal && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                        <div className="bg-[#1A2036] p-6 rounded-lg shadow-lg max-w-md w-full mx-4">
                            <h3 className="text-white text-lg font-semibold mb-4">Rejection Reason</h3>
                ...
                showVehicleModal && selectedVehicle && (
                    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-60 z-50">
                        <div className="bg-[#1A2036] p-6 rounded-lg shadow-xl border border-[#2C3347]/50 max-w-md w-full mx-4 relative">
                            ...
                            {/* Modal Title */}
                            <h3 className="text-white text-2xl font-bold text-center mb-4">Vehicle Details</h3>

                            {/* Vehicle Information */}
                            <div className="text-gray-400 space-y-2">
                                <p><strong>Owner:</strong> {selectedVehicle.nameOfOwner}</p>
                                <p><strong>Address:</strong> {selectedVehicle.addressOfOwner}</p>
                                ...
                                <p><strong>License Plate:</strong> {selectedVehicle.numberPlate}</p>
                                <p><strong>Reg Date:</strong> {new Date(selectedVehicle.regDate).toLocaleDateString('en-GB')}</p>
                                <p><strong>Exp Date:</strong> {new Date(selectedVehicle.expDate).toLocaleDateString('en-GB')}</p>
                                <p><strong>Insurance Provider:</strong> {selectedVehicle.insuranceProvider}</p>
                                <p><strong>Policy Number:</strong> {selectedVehicle.policyNumber}</p>
                                {selectedVehicle.category ?
                                    <p><strong>Category :</strong> {selectedVehicle.category}</p>
                                    :
                                    <div className="w-full h-4 mt-2 mb-6 ">

                                        <label htmlFor=""><strong>Choose a vehicle category :</strong> </label>
                                        <select onChange={(e) => setVehicleCategory(e.target.value)}
                                            name="Category"
                            ...
                            <div className="flex justify-center gap-3 mt-4">
                                {['frontView', 'rearView', 'interiorView'].map((view) => (
                                    <img
                                        key={view}
                                        src={(selectedVehicle.vehicleImages as Record<string, string>)[view]}
                                        alt={`${view} view`}
                                        className="w-24 h-24 rounded-lg object-cover border border-[#2C3347] cursor-pointer"
                                        onClick={() => setSelectedImage((selectedVehicle.vehicleImages as Record<string, string>)[view])}
                                    />
                                ))}
                            </div>
                            ...
                            {/* Action Buttons */}
                            {
                                selectedVehicle.status == 'pending' ? (
                                    ...
                                    <p className="text-white font-bold mt-3">Vehicle has been {selectedVehicle.status == 'approved' ? 'approved' : 'rejected '} successfully</p>
                                )
                            }
