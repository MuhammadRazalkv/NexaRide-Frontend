...
import { login } from "../../../api/auth/driver";
  ...
  const handleSubmit = async () => {
    ...
    if (email && password) {
      ...
      try {
        ...
        setLoading(false);
        console.log("Res.driver", response.driver);

        if (response && response.accessToken && response.driver) {
          const token = response.accessToken;
          console.log("Token ", token);

          const driver = response.driver;
          dispatch(loginSuccessDriver({ token, driverInfo: driver }));
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (response) => {
      ...
      const userSerializable = {
        ...
        profilePic: userInfo?.data?.picture as string,
      };

      try {
        const res = await loginWithGoogle(userSerializable);
        if (res && res.accessToken && res.driver) {
