...
const RatingModal:React.FC<RatingProps> = ({handleChange,handleComments,closeModal,submitReview,error}) => {
    
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative animate-fade-in space-y-5">
        <h2 className="text-2xl font-semibold text-center text-gray-800">Give Feedback</h2>
          {error && (
            <p className="text-sm text-red-500">{error}</p>
          )}
        <div className="flex flex-col items-center gap-4">
          <Rate allowHalf defaultValue={0} style={{ fontSize: "36px" }} allowClear={false} onChange={handleChange} />
          <textarea
        ...
        <div className="flex justify-end pt-2 gap-2">
          ...
          <button className="px-4 py-2 rounded-lg bg-black hover:bg-neutral-800 text-white"
